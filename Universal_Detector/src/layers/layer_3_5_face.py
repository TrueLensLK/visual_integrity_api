...
def analyze_face_consistency(image_path):
    ...
    try:
        ...
        # 2. Detect Face
        # We use the built-in OpenCV Face Detector (Haar Cascade)
        ...
        if len(faces) == 0:
            ...
            return 0 # Neutral (could be a landscape)
            
        print(f"   [Layer 3.5] Found {len(faces)} face(s). Analyzing consistency...")

        # 3. Analyze First Face Found (Main Subject)
        x, y, w, h = faces[0]
        
        # EXTRACT: Face Region (The Suspect)
        face_roi = gray[y:y+h, x:x+w]
        
        # EXTRACT: Background Region (The Reference)
        # We take a patch from the top left (usually background)
        ...
        # 4. Compute Noise Variance (Laplacian)
        ...
        # 5. Calculate Discrepancy (The Trap)
        # A real photo has relatively consistent noise across the image.
        # A Face Swap often has a BLURRY face on a SHARP background (or vice versa).
